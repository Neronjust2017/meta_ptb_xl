...
class ToTensor(object):
    ...
    def __call__(self, sample):
        def _to_tensor(data, transpose_data1d=False):
            if (len(
                    data.shape) == 2 and transpose_data1d is True):  # swap channel and time axis for direct application of pytorch's 1d convs
                data = data.transpose((1, 0))
            if (isinstance(data, np.ndarray)):
                return torch.from_numpy(data)
            else:  # default_collate will take care of it
        ...
        return data, label, ID  # returning as a tuple (potentially of lists)
