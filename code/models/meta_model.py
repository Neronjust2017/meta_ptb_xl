...
class MetaModel(ClassificationModel):
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        data_loader_train, data_loader_valid, loss_fn, model, vcnn = self._get_learner(X_train, y_train, X_val, y_val)

        optimizer = Adam(model.params(), lr=self.lr)
        optimizer_subnet = Adam(model.params(), lr=1e-3)
    ...
    def predict(self, X, Y):
        ...
        data_loader, data_loader_tmp, loss_fn, model, _ = self._get_learner(X, Y, X, Y)
    ...
    def _get_learner(self, X_train, y_train, X_val, y_val, num_classes=None):
        ...
        return data_loader_train, data_loader_valid, loss, model, subnet
