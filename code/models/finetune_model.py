...
# class FinetuneModel(ClassificationModel):
...
class FinetuneModel(ClassificationModel):
    ...
    def fit(self, X_train, y_train, X_val, y_val, X_test, y_test):
        ...
        for epoch in range(self.epochs):
            for batch_idx, (data, target, id) in enumerate(data_loader_valid):  # valid
            ...
            with torch.no_grad():
                ...
                valid_target = []
                valid_pred = []
                for batch_idx, (data, target, id) in enumerate(data_loader_valid):
                    data, target = data.to(device), target.to(device)
                    ...
                    valid_loss += loss.detach().cpu().mean()
                ...
                print('Train Epoch: {} Meta Loss: {:.6f} Test Loss: {:.6f} Meta Metric: {:.6f} Test Metric: {:.6f}'.
                      format(epoch, valid_loss / len(data_loader_valid), test_loss / len(data_loader_test),
                             valid_metric['macro_auc'],  test_metric['macro_auc']))
    ...
    def _get_learner(self, X_train,y_train, X_val, y_val, X_test, y_test, num_classes=None):
        ...
        model = load_checkpoint(model, str(self.outputfolder).replace('finetune', 'baseline', 1).replace('finetune', 'your', 1))
